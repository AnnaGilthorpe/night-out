          ...
          <dl className="mt-10 max-w-xl space-y-8 text-base leading-7 text-gray-600 lg:max-w-none">
                {locations.map((locations) => (
                  ...
                  <div key={locations.name} className="relative pl-9">
                    ...
                    <button type="button" onClick={function location(event) {
                              event.preventDefault()
                              setCode (locations.code)
                             setBook (locations.codeBook)
                              setClick("click")
          }}
                              className="rounded-full bg-neutral-900 py-2 px-3.5 font-com text-sm capitalize text-white shadow shadow-black/60">Book Now </button>
                  </div>
                  </div>
                  </div>
                ))}
